...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/compare-text/sw.js, scope=/compare-text/）。
    // scope がツールの全 locale ページ（/compare-text/, /compare-text/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/compare-text/sw.js', {
      scope: '/compare-text/',
    });
