...
test.describe('covenants', () => {
  ...
  test('links to related tools and its own engineering-notes article (#177)', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(href, `related card ${i} is not a self-link`).not.toContain('/compare-text/');
    ...
    if (await blogLink.count()) {
      await expect(blogLink).toHaveAttribute('href', /^https:\/\/runlocally\.app\/blog\/compare-text\/$/);
    }
  });
  ...
  test('keeps no input text in the page URL after comparing (#7)', async ({ page }) => {
    await page.goto('/compare-text/');
  ...
  test('compares offline after the first online visit (#2)', async ({ page }) => {
    ...
    await page.reload();
    await waitReady(page);
    // Compare online so every JS chunk (incl. the diff-library chunk) is cached too.
    await runComparison(page);
    ...
    try {
      ...
      await waitReady(page);
      await runComparison(page); // compare with no network at all
    } finally {
